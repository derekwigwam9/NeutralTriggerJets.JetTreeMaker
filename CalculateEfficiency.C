...
void CalculateEfficiency(const Bool_t isInBatchMode=false) {
  ...
  // io parameters
  const TString sOutput("pp200r9.cutResponse.d9m2y2018.root");
  ...
  for (UInt_t iTrg = 0; iTrg < NTrgs; iTrg++) {
    for (UInt_t iCut = 0; iCut < NCut; iCut++) {
      hPhiTrk[iTrg][iCut] -> Scale(1. / (Double_t) nTrg[iTrg]);
      hEtaTrk[iTrg][iCut] -> Scale(1. / (Double_t) nTrg[iTrg]);
      hPtTrk[iTrg][iCut]  -> Scale(1. / (Double_t) nTrg[iTrg]);
    }
  }

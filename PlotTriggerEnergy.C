...
void PlotTriggerEnergy() {
  ...
  // grab trees
  TTree *tPi0 = (TChain*) fInPi0 -> Get(sTree.Data());
  TTree *tGam = (TChain*) fInGam -> Get(sTree.Data());
  if (!tPi0 || !tGam) {
  ...
  // fill histograms
  const TString sLeaf("EventList.EClustEneT0 * TMath::Sin(2. * TMath::ATan(TMath::Exp(-1. * EventList.EClustetav1)))");
  const TString sCuts("(EventList.ETwradc11 <= 6004) && (EventList.EEstrpen4 >= 0.5) && (EventList.EPstripenp4 >= 0.5) && (EventList.ETwrPTower < 3.) && (TMath::Abs(EventList.ETwreT) < 0.9)");
  const TString sTspP("((EventList.Etsp > 0.) && (EventList.Etsp < 0.08))");
  const TString sTspG("((EventList.Etsp > 0.2) && (EventList.Etsp < 0.6))");
  ...
  sCutsPi.Append(" && ");
  sCutsGa.Append(" && ");
  sCutsPi.Append(sTspP.Data());
  sCutsGa.Append(sTspG.Data());
  if (doEtCut) {
